function checkUserFile(file, filetype, upload) {
    ...
    if (filetype === 'geojson') {
        ...
        var msg = 'The file must be in GeoJSON format.';
    } else if (filetype === 'multipoints') {
        var file_type = ['text/csv'];
        var msg = 'The coordinates file must be in CSV format.';
    } else {
        return false;
    }
...
function checkSHPLayer(files, upload) {
    ...
    var msg4files = 'Require exactly 4 files with extensions (.shp, .shx, .dbf and .prj).';
    if (files.length != 4) {
    ...
    if (!allEquals(fnmex.names)) {
        flashMessage('The 4 file names must be the same.', 'error');
        $('#selectedUserFiles').empty();
        return false;
    }
...
function uploadUserFiles(colname, multiple) {
    $('#uploadUserFile-' + colname).on('change', function() {
        ...
        if (DATA_USERS[colname].indexOf(ufile) !== -1) {
            msg = msg + ufile + ' is already in the list.\nWould you like to replace it?';
            if (!confirm(msg)) {
    ...
    $('#uploadUserBtn-' + colname).on('click', () => {
        ...
        var endpoint = create_endpoint_post('upload_user_files', 'auth');
        $.ajax({
            type: 'POST',
            url: endpoint,
            data: data,
