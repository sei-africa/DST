...
function displayUserList(pageCtrl) {
    ...
    // var endpoint = create_endpoint_post('getUserList', 'auth');
    $.ajax({
        type: 'POST',
        url: endpoint,
        dataType: 'json',
...
function editUserForm(username, pageCtrl) {
    var endpoint = create_endpoint_get('getUserInfo', 'auth');
    $.ajax({
        dataType: "json",
        ...
        success: (json) => {
            ...
            (function(next) {
                ...
                $('#btn-delete-user').on('click', function() {
                    var txt1 = 'Are you sure to delete permanently the user: ';
...
function validateUserForm(divParent, pageCtrl) {
    ...
    if (pwdCheck) {
        ...
        if (pwd1.length < 4) {
            var text = 'The password must be at least 4 characters.';
            dispMsgUserForm(divP + 'password', text);
            return false;
        }

        if (pwd1 != pwd2) {
            var text = 'Passwords did not match.';
    ...
    if (expiry == '') {
        var text = 'Invalid account expiration date.';
    ...
    if (expiry < today) {
        var text = 'Account expiration date must be in the future';
        // flashMessage(text, 'error');
        dispMsgUserForm(divP + 'expiry', text);
        return false;
    }
    ...
    if (role === 'user') {
        ...
        if (extract.every(x => x === false)) {
            ...
            var text = 'Extraction Support: at least one option must be checked.';
            // flashMessage(text, 'error');
            dispMsgUserForm(id, text);
            return false;
        }
...
function createUserPost(divParent, pageCtrl, userID) {
    ...
    // var endpoint = create_endpoint_post('createUser', 'auth');
...
function createModalUserReply(title) {
    ...
    var divTmp = $('<div>')
        ...
        .appendTo(divModal);
    var divDialog = $('<div>')
        .addClass('modal-dialog modal-dialog-centered vertical-align-center')
        .appendTo(divTmp);
    var divContent = $('<div>')
...
function attachModalUserReply(divParent, dataUser) {
    if (dataUser.update) {
        var title = 'User Information Updated';
    } else {
...
function deleteUser(uid) {
    var endpoint = '/deleteUser';
    // var endpoint = create_endpoint_post('deleteUser', 'auth');
    $.ajax({
        type: 'POST',
        url: endpoint,
        dataType: 'json',
